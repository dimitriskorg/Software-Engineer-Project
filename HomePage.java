...
public class HomePage extends JFrame {
    ...
    public HomePage(User user, Connection conn) {
        ...
        JButton financialStatsBtn = new JButton("Οικονομικά Στατιστικά");
        JButton orderPrepBtn = new JButton("Λίστα Παρασκευής Παραγγελιών");
        JButton inventoryBtn = new JButton("Διαχείριση Αποθεμάτων πρώτων υλών");

        // Add action listeners to the new buttons
        financialStatsBtn.addActionListener(e -> openFinancialReportScreen());
        orderPrepBtn.addActionListener(e -> openOrderPreparationScreen());
        inventoryBtn.addActionListener(e -> openInventoryManagementScreen());

        // Set visibility/enabled state based on user role
        String role = user.getRole();
        if (role.equals("Manager") || role.equals("Accountant")) {
            panel.add(financialStatsBtn);
        }
        if (role.equals("ProductionEmployee")) {
            panel.add(orderPrepBtn);
        }
        if (role.equals("InventoryEmployee")) {
    ...
    private void openFinancialReportScreen() {
        if (user.getRole().equals("Manager") || user.getRole().equals("Accountant")) {
            new FinancialReportUI(conn).setVisible(true);
        } else {
            JOptionPane.showMessageDialog(this, "Δεν έχετε δικαίωμα πρόσβασης στα οικονομικά στατιστικά.");
        }
    }

    private void openOrderPreparationScreen() {
        if (user.getRole().equals("ProductionEmployee")) {
            try {
                ProductionEmployee prodEmp = ProductionEmployee.loadByUserID(conn, user.getUserID());
                if (prodEmp != null)
                    new OrderPreparationUI(conn, prodEmp).setVisible(true);
                else
                    JOptionPane.showMessageDialog(this, "Δεν βρέθηκε προφίλ υπαλλήλου παραγωγής.");
            } catch (SQLException ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(this, "Σφάλμα κατά το άνοιγμα της οθόνης παραγωγής.");
            }
        } else {
            JOptionPane.showMessageDialog(this, "Δεν έχετε δικαίωμα πρόσβασης στη λίστα παρασκευής παραγγελιών.");
        }
    }

    private void openInventoryManagementScreen() {
        if (user.getRole().equals("InventoryEmployee")) {
            try {
                InventoryEmployee invEmp = InventoryEmployee.loadByUserID(conn, user.getUserID());
                if (invEmp != null)
                    new InventoryManagementUI(conn, invEmp).setVisible(true);
                else
                    JOptionPane.showMessageDialog(this, "Δεν βρέθηκε προφίλ υπαλλήλου αποθήκης.");
            } catch (SQLException ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(this, "Σφάλμα κατά το άνοιγμα της οθόνης αποθεμάτων.");
            }
        } else {
            JOptionPane.showMessageDialog(this, "Δεν έχετε δικαίωμα πρόσβασης στη διαχείριση αποθεμάτων.");
        }
    }
